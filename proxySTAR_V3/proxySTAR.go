...
*/
		...
		//Stores everything but the renewal_uri, which will be stored by post_completionURL
...
func storeIssuedCerts (validity int) {
    ...
    //var uriFileName = "renewal_uri" + strconv.Itoa(cronTaskID)
...
func post_completionURL (id, lifetime int, uri string) {
        ...
        renewal_uri = (string)(serialST)

	


	//Stores renewal_uri
    	certDirName := "certificateN" + strconv.Itoa(id)
    	i, err := os.Create("/root/starCerts/" + certDirName + "/renewal_uri")
    	if err != nil {
    	...
    	i.WriteString(renewal_uri)
         ...
         a := successfull_cert{status: "valid", lifetime: lifetime,
             certificate: "https://CertificateAuthoritySTAR:9898/" + renewal_uri}
