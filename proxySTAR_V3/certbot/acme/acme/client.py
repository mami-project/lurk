...
class Client(object):  # pylint: disable=too-many-instance-attributes
	...
	print('LOG from client.py in regr_')
        if 'terms-of-service' in response.links:
            terms_of_service = response.links['terms-of-service']['url']

	#Added in order to check if STAR is supported
	#print('This is the full response')
	#print(response)
	#print('This are the response links')
	#print(response.links)
	#if 'star-certs-supported' in response.links:
	 #   star_certs_supported = response.links['star-certs-supported']['active']
	
        return messages.RegistrationResource(
            body=messages.Registration.from_json(response.json()),
            uri=response.headers.get('Location', uri),
            terms_of_service=terms_of_service)
	    #min_recurrent_certificate_validity=16666, 
	    #max_recurrent_renewal_period=777777)	   
    ...
    def register(self, new_reg=None):
        ...
        """
        #print('LOG from client.py: registering!!')
    ...
    def request_issuance(self, csr, authzrs):
        ...
        if os.path.isfile("../../STARValidityCertbot"):
                ...
                if os.path.isfile("../../STARLifeTimeCertbot"):
                        ...
                        req = messages.CertificateRequest(csr=csr,recurrent=recurrent,recurrent_cert_validity=recurrent_cert_validity, recurrent_start_date=recurrent_start_date, recurrent_end_date=recurrent_end_date)
                        #os.remove()
        if not recurrent:
    ...
    def fetch_chain(self, certr, max_length=10):
        ...
        """
        chain = []
        logger.debug('This is a log from fetch_chain')
        ...
        while uri is not None and len(chain) < max_length:
            ...
            logger.debug('A vale: %s', a)
            logger.debug('END OF THE NEWS')
...
class ClientNetwork(object):  # pylint: disable=too-many-instance-attributes
    ...
    def _send_request(self, method, url, *args, **kwargs):
        ...
        logger.debug('Received response HERE:\nHTTP %d\n%s\n\n%s',
