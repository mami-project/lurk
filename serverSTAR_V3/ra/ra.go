...
func (ra *RegistrationAuthorityImpl) NewCertificate(ctx context.Context, req core.CertificateRequest, regID int64) (cert core.Certificate, err error) {
	...
	*/	
	...
	/*
		...
		startDateFile := "./starCerts/" + crtUuidStr + "/endDate"
 		...
 		defer g.Close()
		defer e.Close()
                if _, err = g.WriteString(startDateString); err != nil {
                    panic(err)
                }
		if _, err = e.WriteString(endDateString); err != nil {
                    panic(err)
                }
	...
	*/
		...
		/*if _, err = f.WriteString(exString); err != nil {
                    panic(err)
                }*/
		pem.Encode(f, &pem.Block{Type: "CERTIFICATE", Bytes: []byte(cert.DER)})
		
		defer f.Close()
		if _, err = f.WriteString(rootCACrt); err != nil {
		    panic(err)
		}

		//Removes tmpFile

		cantRmIT := os.Remove(tmpFileName)
                if cantRmIT != nil {
                        panic(cantRmIT)
                }
	}
