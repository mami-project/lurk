...
//Type used to save the csr in a file for STAR
type NotCookedCsr struct {
        rawCsr string

}
var starDN = "" //Keeps domain name in STAR
...
func (wfe *WebFrontEndImpl) Directory(ctx context.Context, logEvent *requestEvent, response http.ResponseWriter, request *http.Request) {
	...
	/*fmt.Printf("%v",features.DirectoryMeta)
	fmt.Printf("y el otro: ")
	fmt.Printf("%v",clientDirChangeIntolerant)
	fmt.Printf("FIN META")*/
        //if features.Enabled(features.DirectoryMeta) && !clientDirChangeIntolerant {
			...
			"star-certs-supported":"true",
                        "terms-of-service": wfe.SubscriberAgreementURL,
			"min-recurrent-certificate-validity":strconv.Itoa(86400),
			"max-recurrent-renewal-period":strconv.Itoa(31536000),
                }
        //}
...
func (wfe *WebFrontEndImpl) verifyPOST(ctx context.Context, logEvent *requestEvent, request *http.Request, regCheck bool, resource core.AcmeResource) ([]byte, *jose.JSONWebKey, core.Registration, *probs.ProblemDetails) {
        ...
        //fmt.Printf("WFE.GO: Payload value is %s end", payload)
        sizeOfPayload := len(payload)
        if strings.Contains(string(payload[:sizeOfPayload - 1]), "\"recurrent\": true") {

                //fmt.Printf("WFE:GO Domain name is, %v", starDN)

                //Keeps the domain name for STAR auto renewal
                toFileErr := ioutil.WriteFile("STARDomainWFE", []byte(starDN), 0644)
                if toFileErr != nil {
                        panic(toFileErr)
                }

                subStr1 := strings.SplitN(string(payload[:sizeOfPayload - 1]), "recurrent-certificate-validity\": ", 2)
                subStr2 := strings.SplitN(subStr1[1], ",", 3)
                middleOp,noAtoi := strconv.Atoi(subStr2[0])
                if noAtoi != nil {
                        panic(noAtoi)
                }
                middleOp = middleOp/60
                //fmt.Printf("WFE.GO: middleOp is: %v", middleOp)
                subStr2[0] = strconv.Itoa(middleOp)
		...
		*/
		...
		var uuidSTAR string = uuid.NewV4().String()
                ...
                //Hour format UTC
                subStr10 := strings.SplitN(string(payload[:sizeOfPayload - 1]), "recurrent-end-date\": ", 2)
                subStr11 := strings.SplitN(subStr10[1], ",", 3)
		
		//Time is received malformed with an extra "" so they are removed
		subStr4[0] = strings.SplitN(subStr4[0], "\"", 3)[1]
		subStr11[0] = strings.SplitN(subStr11[0], "\"",3)[1]
		//subStr4[0] contains startdate. subStr11[0] contains enddate, meaning together they are the lifetime
		timeformat := "2006-01-02T15:04:05Z"
                timing1, err1 := time.Parse(timeformat, subStr4[0])
		if err1 != nil{
			panic(err1)
		}
                timing2,err2 := time.Parse(timeformat, subStr11[0]) 
		if err2 != nil{
			panic(err2)
		}
                result := timing2.Sub(timing1) //End_date - start_Date

                renewalLifeTime := strings.SplitN(result.String(), "h", 2)[0]
                renewalLifeTimeInt, _ := strconv.Atoi(renewalLifeTime)
		
		//fmt.Printf("WFE.GO: THESE ARE THE TIME LOGS, substr4[0]=%v, substr11[0]=%v, timing2=%v, timing1=%v, result=%v", subStr4[0], subStr11[0], timing2, timing1, result)
		//fmt.Printf("WFE.GO: SECOND PART OF THE TIME LOGS, renewalLifeTimeInt=%v, renewalLifeTime=%v", renewalLifeTimeInt, renewalLifeTime)
                subStr4[0] = strconv.Itoa(renewalLifeTimeInt / 24)
		//fmt.Printf("WFE.GO: FINAL PART OF THE TIME LOGS, subStr4[0] =%v", subStr4[0])
                //End hour format


                var req NotCookedCsr  //keeps the csr in a struct to save later
                req.rawCsr = subStr9[0]

                //fmt.Printf("WFE.GO: Full csr is: %v", subStr9[0])
		//fmt.Printf("WFE.GO: PLEASE HELLO LIFETIME %v TYTY", subStr2[0])
                //_, newErr := os.Create("STARValidityWFE")
                toFileErr = ioutil.WriteFile("STARValidityWFE", []byte(subStr2[0]), 0644)
                if toFileErr != nil {
                        panic(toFileErr)
                }
		//fmt.Printf("WFE.GO: PLEASE HELLO %v TY", subStr4[0])
                toFileErr = ioutil.WriteFile("STARLifeTimeWFE", []byte(subStr4[0]), 0644)
                if toFileErr != nil {
                        panic(toFileErr)
                }
    ...
    if _, err := os.Stat("./starCerts/links"); os.IsNotExist(err) {
        err := os.Mkdir("./starCerts/links", 0777)
         if err != nil {
                panic(err)
         }
    }
